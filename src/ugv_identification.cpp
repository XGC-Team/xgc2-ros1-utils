    ...
    bool operator()(double const* const* parameters, double* residuals) const {
        ...
        for (size_t i = 0; i < poses.size(); ++i) {
            if (i > 0) {
                ...
                state = stepActuatorAndUnicycle(state,
                                                command.linear_velocity_cmd,
                                                command.yaw_rate_cmd,
                                                actuator,
                                                dt);
            }
            const PoseSample predicted = markerPoseFromBodyState(state, body_to_marker);
            const Eigen::Vector4d residual = poseResidual(poses[i],
                                                          predicted,
                                                          options.pose_position_weight,
                                                          options.pose_yaw_weight);
        ...
        residuals[offset++] = options.extrinsic_yaw_prior_weight *
                              normalizeAngle(body_to_marker.rpy.z() -
...
bool validateCommandSamples(const std::vector<CommandSample>& commands, std::string* error) {
    if (commands.empty()) {
        if (error) *error = "command samples are empty";
        return false;
    }
    for (size_t i = 0; i < commands.size(); ++i) {
        ...
        if (!finite(sample.time_s) || !finite(sample.linear_velocity_cmd) ||
            ...
            if (error) *error = "command sample contains non-finite value";
            return false;
        }
        if (i > 0 && sample.time_s <= commands[i - 1].time_s) {
            if (error) *error = "command timestamps must be strictly increasing";
            return false;
        }
    }
    return true;
...
bool validatePoseSamples(const std::vector<PoseSample>& poses, std::string* error) {
    if (poses.size() < 2) {
        if (error) *error = "at least two pose samples are required";
        return false;
    }
    for (size_t i = 0; i < poses.size(); ++i) {
        ...
        if (!finite(sample.time_s) || !finiteVector(sample.position) ||
            ...
            if (error) *error = "pose sample contains invalid value";
            return false;
        }
        if (i > 0 && sample.time_s <= poses[i - 1].time_s) {
            if (error) *error = "pose timestamps must be strictly increasing";
            return false;
        }
    }
    return true;
...
double firstOrderExactStep(double state, double command, double gain, double tau_s, double dt_s) {
    if (!finite(state) || !finite(command) || !finite(gain) || !finite(tau_s) ||
        !finite(dt_s) || tau_s <= 0.0 || dt_s < 0.0) {
        return state;
    }
...
BodyState stepActuatorAndUnicycle(const BodyState& state,
    ...
    next.linear_velocity = firstOrderExactStep(state.linear_velocity,
                                               linear_velocity_cmd,
                                               actuator.k_v,
                                               actuator.tau_v,
                                               dt_s);
    next.yaw_rate = firstOrderExactStep(state.yaw_rate,
                                        yaw_rate_cmd,
                                        actuator.k_w,
                                        actuator.tau_w,
                                        dt_s);
...
PoseSample markerPoseFromBodyState(const BodyState& body, const ExtrinsicEstimate& body_to_marker) {
    ...
    pose.position = Eigen::Vector3d(body.x, body.y, 0.0) +
                    world_body_rotation * body_to_marker.xyz;
...
Eigen::Vector4d poseResidual(const PoseSample& measured_marker_pose,
    ...
    residual.head<3>() = position_weight *
                         (predicted_marker_pose.position - measured_marker_pose.position);
...
IdentificationMetrics computeMetrics(const std::vector<PoseSample>& measured,
    ...
    for (size_t i = 0; i < measured.size(); ++i) {
        ...
        const double yaw_error =
            normalizeAngle(yawFromQuaternion(predicted[i].orientation) -
    ...
    metrics.final_position_error =
        (predicted.back().position - measured.back().position).norm();
...
IdentificationResult identifyActuatorAndExtrinsic(const std::vector<CommandSample>& commands,
    ...
    double initial[5] = {
        result.initial_state.x,
        result.initial_state.y,
        result.initial_state.yaw,
        result.initial_state.linear_velocity,
